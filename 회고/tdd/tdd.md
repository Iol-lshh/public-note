...
애플리케이션 코드를 온전히 바이브 코딩하기엔 불안하다.애플리케이션 코드를 AI에 의존해 전적으로 작성하는 건 여전히 불안하다. 비즈니스 로직과 직결되기 때문이다. 하지만 테스트는 비교적 부담이 적다. 결정적인 장애로 이어지지 않고, 설령 품질이 낮더라도 나중에 참고하여 새로 작성할 수 있기 때문이다. 물론 지나치게 의존성이 많고 유지보수가 어려운 '나쁜 테스트'는 피해야 하지만, **테스트는 없는 것보다, 못생긴게 훨씬 낫다**.
...
테스트 코드가 TDD로만 이루어진 것은 아니다. 테스트 코드의 본질적인 목적은 **회귀 방지**와 **인수**다. 이에 대해서는 개략적으로 [이전 글](https://iol-lshh.github.io/회고/test/테스트_정리/)에서 정리했다.
...
1. 원하는 결과물 샘플을 제공한다.
2. 원하는 요구사항을 제공한다.
3. 결과물을 수정하며 적절한 체인을 쌓아 올린다.

이런 방식에 따라 테스트 코드를 작성하는데 여러 IDE와 여러 AI 툴들이 있겠지만, 나는 커서를 사용하고 있다. 커서와 함께 바이브 코딩을 진행했다.
...
여기서 느낄 수 있는 주의점이 있다. 유스케이스 테스트는 **블랙박스 테스트** 형태를 지향해야 한다. 위키백과에서 각각의 정의를 가져왔다.
...
**화이트박스 테스트**는 일반적으로 블랙박스 테스트에 비해 회귀 방지에 좋지만, 리팩터링 내성에 취약하며, **블랙박스 테스트**는 이와 반대다. TDD의 목적은 요구사항을 리팩터링해가며 구현하는 것이다. 내부가 어떻게 변화될지 모른다. 최대한 블랙박스 테스트를 지향하여 작성해야 한다.
...
도메인 모델로 가는 것은 리펙토링에 해당한다고 생각한다. 루퍼스 과제는 요구사항이 기능 구현과 도메인 모델에서 필요한 것이 관심사에 따라 명확하게 제공된다. 하지만 현실세계 실무는 요구사항을 줄 때 관심사가 구분되지 않는다.

때문에 이를 생각해서 유스케이스에 대한 블랙박스 테스트로 우선 작성하고, 도메인 모델과 도메인 테스트를 작성했다.

이 때에는 모든 공유 자원들을 떼어내고 단위 테스트로써 작성했다.
