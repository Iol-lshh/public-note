...
그럼에도 불구하고 성능은 그리 좋아지지 못했다. 이를 해결하기 위한 다른 방안이 있을 것이다. 다만 최대한 심플한 해결책을 적용해야 유지보수의 성능을 높이고, 결함 비율을 낮출 수 있다. 그런 방법 중 하나가 캐시라고 생각한다.
...
조회 성능에 대한 실험을 수행하고 결과를 기록했다. 다음엔 쓰기 성능과 MV와 같은 것이 목표가 되지 않을까 싶다.