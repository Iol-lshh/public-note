...
조금 더 페더스가 말한 개선 방법을 자세히 들여다보자. 페더스는 먼저 "코드의 변경" 그 자체를 낱낱이 해부했다.
...
**Legacy Code를 개선한다는 것은 여러 코드 변경 행위 중, "리팩터링"한다**는 것을 의미한다는 것을 정리했다. 리팩터링은 동작 변경 없이 설계를 개선하는 것, 즉 **의미 보존**(semantic-preserving)이 이루어져야 한다고 했다. 

또한 'Refactoring'(fowler, 2020)에서 파울러는 이렇게 말했다.

> 리팩터링의 궁극적인 목적은 개발 속도를 높여서, 더 적은 노력으로 더 많은 가치를 창출하는 것이다.

그러니깐 리팩터링 목적은 어디까지나 **작업의 효율화**다. 때문에 파울러는 다음 경우가 리팩터링하지 말아야 할 때라고 하고 있다.
