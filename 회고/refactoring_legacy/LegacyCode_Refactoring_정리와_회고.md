...
어떤 방식이든 시간이 지남에 따라 비즈니스 로직이 쌓이면 **기술 부채**(technical debt)가 생길 수 있고, 코드를 개선하는 일은 중요한 과제가 된다는 것이다. 그럼 어떻게 개선해야 할까? 두가지 단어를 떠올렸다.
...
> 조임 지점의 테스트 루틴은 숲에 걸어 들어가서 줄을 그은 다음 '이 구역은 전부 내 것'이라고 말하는 것과 비슷하다. 해당 구역을 완벽히 숙지한 후에 비로소 리팩토링 및 테스트 루틴을 작성함으로써 더욱 발전시킬 수 있는 것이다. 마침내 조임 지점의 테스트 루틴을 삭제하고 각각의 클래스에 대한 단위 테스트를 사용해 개발을 진행할 수 있게 된다. - Working Effectively with Legacy Code (Feathers, 2004)
...
> 문서화 테스트는 대체로 '화창한 날' 테스트다. 특별한 조건을 테스트하지 않고, **특정 동작이 유지되는지 검증**할 뿐이다. 문서화 테스트를 통해 코드를 이동하거나 추출한 리팩토링이 기존 동작을 유지한다고 유추할 수 있다. - Working Effectively with Legacy Code (Feathers, 2004)
