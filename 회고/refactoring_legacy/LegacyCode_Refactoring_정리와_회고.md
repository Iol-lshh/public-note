...
> 문서화 테스트는 대체로 '화창한 날'(good weather) 테스트다. 특별한 조건을 테스트하지 않고, **특정 동작이 유지되는지 검증**할 뿐이다. 문서화 테스트를 통해 코드를 이동하거나 추출한 리팩토링이 기존 동작을 유지한다고 유추할 수 있다. (13) - Working Effectively with Legacy Code (Feathers, 2004)
