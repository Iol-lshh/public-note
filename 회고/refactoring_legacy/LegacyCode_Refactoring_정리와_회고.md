...
P of EAA에서 파울러는 비즈니스 로직 계층에 대한 패턴으로 트랜잭션 스크립트 패턴과 도메인 모델 방식을 이야기한다. 트랜잭션 스크립트 방식은 유스케이스에 대해 절차적으로 비즈니스 로직을 풀어나가는 방식이고, 도메인 모델은 DDD에서 이야기하는 코드가 도메인이 되는 방식이다.
...
그렇다면 트랜잭션 스크립트 방식으로 가야 하는가? 트랜잭션 스크립트 방식은 많은 회사에서 쓰이는 방식이다. 또한 절차적인 비즈니스 로직의 구현 묶음이라 읽기에는 쉽다. 하지만 단점은 매우 명백하다. 현실 세계는 혼잡하다. 비즈니스 로직이 발전되며 트랜잭션 스크립트 방식은 **많은 분기와 코드 중복을 유발**하게 되고, **기하급수적으로 복잡성이 커진 시스템은 유지보수 효율과 생산성이 급락**한다.

트랜잭션 스크립트 방식은 초반에 유리하며, 생산성의 향상을 위해서는 적절한 시점에 도메인 모델 방식으로 전환되어야 하며, 도메인 모델을 도메인 전문가들과 소통하며지속적으로 변화시켜야 한다. 어떤 방식이든 시간이 지남에 따라 비즈니스 로직이 쌓이면 **기술 부채**(technical debt)가 생기는 것이다. 코드를 개선하는 일이 중요한 과제가 된다는 것이다.

이런 일련의 생각들을 거치며 개선에 대한 실천 방법이 궁금해졌다. 그럼 어떻게 개선해야 할까? 두 가지 단어를 떠올렸다.
...
조금 더 페더스가 말한 개선 방법에 대해 자세히 들여다보자. 먼저 이를 알기 위해서는 레거시 코드의 본질을 파악해야한다. 

---

## 코드 변화의 본질 - Feathers가 말하는 4가지 이유
...
코드의 수정에는 리팩터링을 포함하여 여러가지가 있다는 것을 알았다. 또한 어떻게 코드를 수정해야 하는지, 소프트웨어 바이스에 대해서도 정리했다. 이제 다시 본 글의 주제인, Legacy Code 개선 방법에 대해 본격적으로 정리해보자. 

---

## 리팩터링 - 도덕이 아닌 경제 논리

**Legacy Code를 개선한다는 것은 여러 코드 수정 작업 종류 중, "리팩터링"한다**는 것을 의미한다.
...
코드가 바람직하지 않을때 **악취**(smell)가 난다고 한다. 그렇다면 어떤 것이 바람직하지 않은 코드인가? 개발자가 설마 일부러 악취를 처음부터 만들진 않을 것이다. 소프트웨어는 유지보수 되면서 요구사항에 의해 변화되고, 그러면서 코드는 풍성해지고 빈약해지고 맥락이 바뀌길 반복한다. 그런 **변화는 코드를 의도와 어긋나게 만든다.** 이것이 악취의 본질인 것이다. 다음은 파울러가 말하는 악취의 패턴들과 해결책이다.
