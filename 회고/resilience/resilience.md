...
더구나 장애와 같은 상황에서 하나의 요청이 스레드를 오래 물고 있는 것은 자칫 애플리케이션의 자원 고갈로 이어질 우려가 있다. 이때 고려가능한 해결책들에는 여러가지 있겠지만 가장 기본적인 두 가지에 대해 살펴보자.
...
외부 시스템 연동에 의한 스레드 자원 고갈을 시키지 않기 위한 방법은, 그 연동을 끊는 것이다. 그리고 그러기 위한 가장 간단한 방법은 내 쪽에서 시간을 정해두고 정해진 시간 내에 연동을 실패할 경우, 

타임아웃에는 두 가지 종류가 있다. 연결(connection)과 읽기(read)다.

![](img/http_con_read.png)
...
때문에 Connection Timeout을 설정하는 첫 걸음은, 엔드포인트 간의 최소-최대 RTT가 몇인지 알아야 한다. 이 **'SYN -> SYN + ACK -> ACK' 세 차례의 과정을 계산**할 수 있다. 이 계산이 끝나고 나서야, **Connection을 위해 몇번이나 재시도하게 할까**에 대한 고민이 필요하다. 그 계산이 녹아들어간 것이 Connection Timeout이다.
...
타임아웃을 설정함으로써, 내 애플리케이션이 무작정 응답을 기다리게 되진 않았다. 근데 여기서 다시 고민해볼 것이 있다. 만약 외부 시스템이 다운되어서 동작을 하지 않는다. 이 때에도 Timeout을 무작정 기다리게 하는게 올바를까?

### CircuitBreaker

서킷브레이커는 회로 차단기를 의미한다. 회로가 닫힌(closed) 상태에서 전기가 흐르지만, 회로가 열린(open) 상태에서 전기의 흐름이 차단된다. 이런 구조는 누전기 등에 사용된다.

애플리케이션에서 이런 패턴을 적용할 수 있다. 애플리케이션의 서킷 브레이커는 3가지 상태를 가진다.

- **CLOSED**: 기능하는 상태
- **OPEN**: 기능을 멈춘 상태
- **HALF_OPEN**: 기능을 점검하는 상태

![](img/circuitbreaker.png)

서킷브레이커는 **기능적으로 실패를 캐싱**한다고 볼 수 있다. Timeout을 기다리지 않고, 실패를 캐싱함으로써 애플리케이션이 불필요한 자원 소모를 방지한다.

---

결제의 진정한 트랜잭션(거래)은 우리 서비스도, PG도 아니다. VAN과 카드사에서 일어난다. 우리의 서비스가 모놀리식으로 되어있다. 하지만 전체 결제 과정으로 본다면, 이미 겹겹이 쌓인 마이크로서비스 구조로 각각 관심사는 분리되어 비동기로 진행된다.

이때 데이터의 동기화를 통해 각 마이크로서비스는 협업해야한다. 이런 도메인 흐름을 이해하고, 각 동기화 지점을 구현해야한다.

그리고 이 지점의 구현에서 장애가 서로에게 전파되는 것을 막아야한다. 엔터프라이즈 애플리케이션 아키텍처 패턴은 세가지로 이루어져있다. 비즈니스 모델, 데이터 정합성, 동작. 보통 비즈니스 모델과 데이터의 정합성의 중요함에 대해서는 모두가 쉽게 인지한다. 하지만 애플리케이션의 성능과 장애 방지에 해당하는 동작은 어려운 부분이다. 경쟁사를 이기기 위한 방법이 뭘까? 이 어려운 부분을 공략하는 것이 경쟁력을 갖는 부분일 것이다.
