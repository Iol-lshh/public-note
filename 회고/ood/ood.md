...
불필요한 부분을 잘라내어 **업무 효율화**와 **필요하지만 없었던 프로세스**를 정리한다.
...
- **언어**의 차이: 동일한 작업 객체를 Actor간에 다르게 부른다. 또는 같은 이름이 다른 의미로 사용된다.
- **같은 것**을 **다른 용도**로 사용: 동일한 작업 객체가 완전히 다른 업무에 사용된다. 아래 그림에서 '계약'에 대해 *고객은 '서명'이란 행위*에 사용하고, *위험관리자는 '심사'란 행위*에 사용한다.
...
전술적 설계를 통해 드러나는 것을 '**도메인 모델**'이라고 한다. 도메인 모델은 지식의 압축 형태다. 도메인의 구조적 측면, 개념, 행위와 상호 작용을 모두 드러내야 한다. 이런 개념을 DDD에서는 도메인 모델이 '**불변식**'(invariant)를 갖는다고 한다. 불변식은 로직에서의 객체의 무결성과 일관성이 보장되는 것을 의미한다.

- **불변식**(Invariant): 시스템이 항상 유지해야 하는 핵심 규칙

이런 설계를 위해서는 **동사**(verb)를 봐야한다.

- Actor는 목표를 달성하기위해 **무엇**(what)을 하고 있는가?
- Actor는 작업 객체를 **어떻게 처리**하는가? (절차, 전략, 행위 중심)
	- *고객 정보*를 검증 프로세스를 통해 **검토**하고 **승인**한다.
- Actor는 **어떤 방법**으로 작업 객체를 **다루는가**? (도구, 수단, 방식 중심)
	- *고객 정보*를 **검증 프로세스를 통해** 검토하고 승인한다.
- Actor는 어떻게 정보를 교환하고 **협력**하는가?

다음은 위 1, 2, 3, 4, 5 로 이루어진 서브 도메인을 도메인 모델링하는 과정이다.
...
- **작업 객체**는 도메인 모델에서 애그리거트(aggregate), 엔터티(Entity), 값 객체(Value Object)가 되는 후보다.
- **활동**은 연산(메서드, 함수, 명령)이 될 수 있다. 활동의 결과는 도메인 이벤트로 구현될 수도 있다.

애그리거트, 엔터티, 값 객체에 대해 간단히 이야기해보면 다음과 같다.
...
이 글 앞서 객체에 대해 많은 이야기를 했으므로, 볼드표시로 하고 설명은 생략한다. 대신 도메인 설계 관점에서 애그리거트에 대해 조금 더 이야기 해보겠다. 

애그리거트라는 경계를 두는 이유는 단순하다. 비즈니스의 흐름은 **불변식**을 가져야한다. 하지만 흐름도가 여러곳에서 오고간다면, 관리가 힘들다. 때문에 흐름을 조이고, 유일하게 만들어, **불변식이 깨지지 않도록** 하는 것이다. 또한 이렇게 유일한 조임 지점을 갖게 될 경우, 동시성과 같은 문제 해결에서 **영향 흐름도가 단순**해지기 때문에 **잠금 지점**이 명확해지는 이점을 얻을 수 있다.
...
이런 상태 기계는 **계약에 의한 설계**(Design by Contract, DbC)로 **선행조건**(precondition)과 **후행조건**(postcondition)의 시각화를 돕는다.
