...
시스템 트랜잭션은 트랜잭션하면 바로 떠오르는 그거다. 동기적으로 ACID로 처리되어야 하는 것. 비즈니스 트랜잭션은 조금 다르다. **최종적 일관성**을 갖고 비동기적으로 처리될 수도 있다. 중요한 것은 **사용자로부터 요청을 일관된 유스케이스 로직을 처리**한다는 것이다.
...
동기/비동기, 블로킹/논블로킹, 동기화 개념까지 살펴봤다면, 이제 이 개념들이 실제 시스템에서 어떻게 조합되는지를 이해해야 한다. 그 핵심이 바로 **비즈니스 트랜잭션**이다.
...
- 결과는 정해진 순서대로 도달해야 한다.
- 결과는 중복되지 않아야 한다. 
- 결과를 되돌릴 수 있어야한다.

시스템 트랜잭션 수준에서는 단순히 락으로 구현되지만, **마이크로서비스에서는 이러한 기술들이 잘 작동하지 않는다.** 그래서 이 동기화의 개념은, **유스케이스 로직 수준으로 올라가야 한다.** 이를 단순히 동기로 처리할 수도 있으나, 직접 세가지 목표를 구현하는 것은 서비스가 복잡해질 수록 난이도가 올라가고, 성능도 떨어질 수 있다.

여기서 등장하는 것이 **사가**(Saga)다.

---

## 사가 (Saga)

사가(Saga)는 동기화 개념을 비즈니스 트랜잭션에서 비동기 메시지 방식으로 실현하는 방식이다. **트랜잭션을 작은 단위의 지역 트랜잭션으로 분리하고**, 각 단계의 성공/실패 여부에 따라 **다음 단계를 진행하거나 보상 처리**하는 **비동기적이면서도 일관성 있는 트랜잭션 설계 방식**이다.
