...
시스템 트랜잭션은 락으로 동기화를 해결하지만, 비즈니스 트랜잭션에서는 한계가 있다. 단일 데이터베이스가 아닌 여러 서비스와 외부 시스템을 아우르는 복잡한 흐름을 다루기 때문이다. 여기서의 동기화는 **단순히 타이밍 조절과 일관성을 넘어, 서비스 간 독립성도 유지해야 한다**. 기존 락 기반 접근으로는 이를 충족하기 어렵기에, 새로운 방안이 필요하다. 바로 사가(Saga)다.
...
(이 글에서는 사가를 아주 간략하게 비즈니스 트랜잭션 관점에서 살펴봤다. 조만간 크리스 리처드슨의 MSA 책을 읽고 정리한 글에서 더 자세히 정리할 것이다.)
